...
use arrow_array::{
    builder::{
        ...
        Int64Builder, Int8Builder, StringBuilder, StructBuilder,
    },
    RecordBatch, RecordBatchReader,
};
use arrow_schema::{ArrowError, DataType, Field, SchemaBuilder, SchemaRef};
use aws_sdk_athena::{
    error::DisplayErrorContext,
    operation::get_query_results::GetQueryResultsOutput,
    types::{ColumnInfo, ColumnNullable, QueryExecutionState, ResultSetMetadata},
    Client,
};
use tokio::{
    runtime::Runtime,
...
impl AthenaClient {
    pub fn new(client: Arc<Client>, runtime: Arc<Runtime>) -> Self {
        Self { client, runtime }
    }
    ...
    pub fn execute(
        ...
        let reader = self.runtime.block_on((async || {
            let query_execution_id = self.run_query(query_string).await?;
            AthenaResultReader::create(
                self.client.clone(),
                self.runtime.clone(),
                query_execution_id,
            )
    ...
    async fn run_query(&self, query_string: &str) -> Result<String> {
...
fn schema_from_metadata(meta_data: &ResultSetMetadata) -> (SchemaRef, Vec<AthenaType>) {
...
mod tests {
    ...
    fn create_athena_client(aws_sdk_client: Client) -> AthenaClient {
        let runtime = Runtime::new().expect("failed to create tokio runtime");
        AthenaClient::new(Arc::new(aws_sdk_client), Arc::new(runtime))
    }

    #[test]
    fn execute_makes_the_athena_query_lifecycle_api_calls() {
        let (client, rules) = athena_happy_path();
        let athena_client = create_athena_client(client);
        let _ = athena_client.execute("SELECT 1");
    ...
    fn execute_returns_the_query_results_as_a_record_batch() {
        ...
        let athena_client = create_athena_client(client);
        let batches: Vec<RecordBatch> = athena_client
            .execute("SELECT 1")
            .unwrap()
